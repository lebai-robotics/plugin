...
def main():
    modbus_address = (lebai.get_item("plugin_yolo_modbus_address"))['value']
    if not modbus_address:
        modbus_address = ""
    modbus_id = (lebai.get_item("plugin_yolo_modbus_id"))['value']
    if not modbus_id:
        modbus_id = "8"
    ...
    yolo_signal = (lebai.get_item("plugin_yolo_signal"))['value']
    if not yolo_signal:
        yolo_signal = "14"
    yolo_signal = int(yolo_signal)
    while True:
        time.sleep(0.1)
        yolo_id = lebai.get_signal(yolo_signal)
        if yolo_id > 0:
            yolo_data = yolo.main()
            lebai.set_item("plugin_yolo_tags", json.dumps(yolo_data))

            lebai.set_signal(yolo_signal, -1)
        if modbus_address != "" and hr_block.getValues(300)[0] == 1:
            hr_block.setValues(301, 0)
            flange_pose = reg2pose(hr_block.getValues(302, 6))
            yolo_data = yolo.find_tags_pose(flange_pose)
            for tag_id, tag in yolo_data.items():
